...
import { shouldShowInterstitial } from './admob';
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  tapArrow: (row, col) => {
    ...
    if (isValidRemoval(state.grid, arrow)) {
      ...
      if (newRemaining === 0) {
        setTimeout(() => {
          // Re-read settings from the live store — the snapshot captured
          // at tap time may be stale if the user toggled sound/vibration
          // during the 400ms animation window.
          const liveSettings = get().settings;
          if (liveSettings.sound) playLevelComplete();
          if (liveSettings.vibration && navigator.vibrate) {
          ...
          if (currentLevel > 0) {
            ...
            // AdMob: every 5 levels after level 20 triggers an interstitial
            const showInterstitial = shouldShowInterstitial(currentLevel);
            set({
              levelComplete: true,
  ...
  watchAdForLife: () => {
    // Real AdMob rewarded ad integration — handled by RewardedAdPlaceholder
    // component which calls showRewardedAd() in admob.ts. The native plugin
    // returns the reward in onUserEarnedReward, and we grant +1 life here.
    const state = get();
    if (state.lives <= 0) {
